...
async function fetchBaseCurrency(inputCurrency) {
  const response = await ExchangeService.getExchangeRates(inputCurrency);
  if (response instanceof Error) {
    ...
    lastReceivedCurrency = [];
    // search or "loop" through result.conversion_rates and locate outputCurrency
    // response.PROPERTY = outputCurrency;
    showExchangeRate(response);
  }
}

function requestIsCached(requestedCurrency, cachedResponses) {
  if (requestedCurrency === cachedResponses[0]) {
...
$(document).ready(function () {
  ...
  $("#currency-submit").on("click", function() {
    ...
    if (!requestIsCached(inputCurrency, cachedResponses)) {
      // if the base unit & time is not yet stored/fetched, fetch it
      // and add to the array of cached responses
      cachedResponses.push(fetchBaseCurrency(inputCurrency));
    }
    // Refactor this to search the array for the correct response (instead of just the most recent one)
    baseCurrency = cachedResponses[cachedResponses.length - 1];
    showExchangeRate(baseCurrency, outputCurrency);
  });
});